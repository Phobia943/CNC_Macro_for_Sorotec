...
;---------------------------------------------------------------------------------------
  ...
  ; Im Rendermodus (#5397=1) nicht ausfuehren
  ; Simulationsmodus (#5380=1) ist OK - zum Testen
  IF [#5397 == 1] THEN
...
;---------------------------------------------------------------------------------------
  ...
  DlgMsg "Werkzeugwechsel-Typ" "0=Aus, 1=Pos, 2=Pos+Mess" 4520

  IF [#5398 == 1] THEN ; OK gedrueckt
    IF [#4520 > 0] THEN ; Wenn Werkzeugwechsel aktiv
      DlgMsg "WZ-Wechsel Position" "X (mm)" 4521 "Y (mm)" 4522 "Z (mm)" 4523
    ENDIF
  ENDIF
...
;---------------------------------------------------------------------------------------
  ...
  DlgMsg "Z-Sensor" "Typ (0=NO, 1=NC)" 4400 "Hoehe (mm)" 4510 "Anfahrt (mm/min)" 4512 "Tasten (mm/min)" 4513
...
;---------------------------------------------------------------------------------------
  ...
  DlgMsg "Pos nach Referenz" "X (mm)" 4631 "Y (mm)" 4632 "Z (mm)" 4633

  ; Werkzeuglaengensensor Parameter
  DlgMsg "WZ-Sensor Pos" "X (mm)" 4507 "Y (mm)" 4508 "Sicher Z (mm)" 4506 "Spindel Z0 (mm)" 4509
  DlgMsg "WZ-Sensor" "Max Laenge (mm)" 4503 "Anfahrt (mm/min)" 4504 "Tasten (mm/min)" 4505

  ; Bruchkontrolle
  DlgMsg "Bruchkontrolle" "Aktiv (0/1)" 4529 "Toleranz (mm)" 4528

  ; Position nach Messung
  DlgMsg "Pos nach Messung" "Funktion (0-4)" 4519 "X (mm)" 4524 "Y (mm)" 4525
...
;---------------------------------------------------------------------------------------
  ...
  DlgMsg "Warmlauf 1+2" "St.1 RPM" 4532 "St.1 Sek" 4533 "St.2 RPM" 4534 "St.2 Sek" 4535
  DlgMsg "Warmlauf 3+4" "St.3 RPM" 4536 "St.3 Sek" 4537 "St.4 RPM" 4538 "St.4 Sek" 4539
...
;---------------------------------------------------------------------------------------
  ...
  IF [#5008 > 97] THEN
    msg "Werkzeug ist 3D-Taster"
    msg "Laengenmessung nicht erforderlich"
    M30
  ENDIF
  ...
  IF [[#5398 == 1] AND [#5397 == 0]] THEN ; OK gedrueckt UND nicht im Rendermodus
    ...
    IF [[#4509 + #5017 + 10] > [#4506]] THEN
      DlgMsg "WARNUNG: Werkzeug zu lang!" "Eingegebene Laenge:" 5017
    ENDIF

    ; Wenn immer noch ungueltige Eingabe: Abbruch
    IF [[#5017 <= 0] OR [[#4509 + #5017 + 10] > [#4506]]] THEN
      ErrMsg "WZ-Messung abgebrochen - Ungueltige Laenge"
    ENDIF
...
;---------------------------------------------------------------------------------------
  ...
  IF [#5398 == 1] THEN ; OK gedrueckt
    ...
    ; Stufe 1
    msg "Warmlauf 1: " #4532 " RPM, " #4533 " s"
    M03 S[#4532]
    G04 P[#4533]

    ; Stufe 2
    msg "Warmlauf 2: " #4534 " RPM, " #4535 " s"
    M03 S[#4534]
    G04 P[#4535]

    ; Stufe 3
    msg "Warmlauf 3: " #4536 " RPM, " #4537 " s"
    M03 S[#4536]
    G04 P[#4537]

    ; Stufe 4
    msg "Warmlauf 4: " #4538 " RPM, " #4539 " s"
    M03 S[#4538]
    G04 P[#4539]
  ...
  IF [[#1200 == 1] AND [#1201 == 1]] THEN
    ...
    IF [#5067 == 1] THEN ; Kante gefunden
      ...
      IF [#5067 == 1] THEN
        ...
        msg "X+ Kante OK, Nullpkt gesetzt (R komp.)"
      ELSE
  ...
  IF [[#1200 == 2] AND [#1201 == 1]] THEN
    ...
    IF [#5067 == 1] THEN
      ...
      IF [#5067 == 1] THEN
        ...
        msg "Y+ Kante OK, Nullpkt gesetzt (R komp.)"
      ELSE
        ErrMsg "FEHLER: Kante nicht gefunden (Feinmessung)"
      ENDIF
    ELSE
              ...
              IF [#5398 == 1] THEN
                ...
                IF [#5067 == 1] THEN
                  ...
                  IF [#5398 == 1] THEN
                    ...
                    IF [#3503 == 1] THEN
                      G68 R[#2007]
                      msg "Nullpkt gesetzt, Rotation " #2007 " Grad"
                    ELSE
                      msg "Nullpkt gesetzt, keine Rotation"
                    ENDIF
      ...
      IF [#5067 == 1] THEN
        ...
        IF [#5067 == 1] THEN
          ...
          IF [#5067 == 1] THEN
            ...
            msg "Lochmitte gefunden"
            msg "Durchm. X=" #107 " Y=" #108 " mm"
      ...
      IF [#5067 == 1] THEN
        ...
        IF [#5067 == 1] THEN
          ...
          IF [#5067 == 1] THEN
            ...
            msg "Zylindermitte gefunden"
            msg "Maschine auf Mittelpunkt"
    ...
    IF [#5067 == 1] THEN ; Sensor gefunden
      ...
      IF [#5067 == 1] THEN ; Sensor erneut gefunden
        ...
        IF [[[#5021 + #4528] > [#4501]] AND [[#5021 - #4528] < [#4501]]] THEN
          ...
          ; Werkzeug OK
          msg "Bruchkontrolle OK"
          msg "Abweichung: " [#5021 - #4501] " mm"
              ...
              IF [#5067 == 1] THEN
                ...
                IF [#5067 == 1] THEN
                  ...
                  IF [#5067 == 1] THEN
                    ...
                    ; === ERGEBNISSE ANZEIGEN ===
                    IF [#2106 == 0] THEN
                      msg "Rechteck-Vermessung OK"
                      msg "Ist X: " #2100 " Soll: " #1100
                      msg "Abw X: " #2104 " mm"
                      msg "Ist Y: " #2101 " Soll: " #1101
                      msg "Abw Y: " #2105 " mm"
                      msg "Mitte X: " #2102 " Y: " #2103
                      msg "Toleranz " #4600 " mm OK"
                    ...
                    ELSE
                      warnmsg "MASSABWEICHUNG ZU GROSS!"
                      msg "X Ist: " #2100 " Soll: " #1100
                      msg "X Abw: " #2104 " mm"
                      msg "Y Ist: " #2101 " Soll: " #1101
                      msg "Y Abw: " #2105 " mm"
                      msg "Max.Tol: " #4600 " mm"
                    ...
                    msg "Setze Nullpkt auf Rechteck-Mitte..."
                    ...
                    msg "Nullpunkt gesetzt"
                    msg "Maschine auf Rechteck-Mitte"
...
;---------------------------------------------------------------------------------------
  ...
  IF [#1120 == 3] THEN
    ...
    msg "Gespeicherte Koord.systeme:"
    msg " "
  ...
  IF [#1120 == 5] THEN
    ...
    msg " "
    msg "Hinweis: Koord.system pruefen"
...
;---------------------------------------------------------------------------------------
  ...
  IF [#5397 == 0] THEN ; Nicht im Rendermodus
    ...
    IF [#5015 == 1] THEN
      ...
      msg "WZ " #5008 " -> WZ " #5011 " gewechselt"
...
;---------------------------------------------------------------------------------------
  ...
  msg "Ersten Punkt antasten, dann STRG+G"
  M0
  #5020 = #5071  ; X1
  #5021 = #5072  ; Y1

  msg "Zweiten Punkt antasten, dann STRG+G"
  M0
  #5022 = #5071  ; X2
  #5023 = #5072  ; Y2
